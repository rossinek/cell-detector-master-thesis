...
\FORALL{$v \gets v_0, u_0$}
  \LOOP
    \STATE
      ...
      \hphantom{$\text{E}_v \gets$} które nie tworzą cyklu w grafie $\mathbf{S}$
    \IF{$\text{E}_v = \O$}
      \STATE \textbf{break}
    \ENDIF
    \STATE $\{v, u\} \gets \textbf{argmax}_{e \in \text{E}_v} \text{q}_v(e)$
    \STATE $\mathbf{S} \gets \mathbf{S} \cup (\{u\}, \{\{v, u\}\}) $
    \STATE $v \gets u$
  \ENDLOOP
\ENDFOR
\end{algorithmic}
gdzie funkcja $\text{q}_v(e)$ jest funkcją oceny która służy do wyboru najmocniejszej związanej krawędzi.

\ldots % TODO Funkcja oceny
